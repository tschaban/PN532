...
int16_t PN532_I2C::getResponseLength(uint8_t buf[], uint8_t len,
                                     uint16_t timeout) {
  const uint8_t PN532_NACK[] = {0, 0, 0xFF, 0xFF, 0, 0};
  uint16_t time = 0;

  do {
    if (_wire->requestFrom(address, 6)) {
...
int16_t PN532_I2C::readResponse(uint8_t buf[], uint8_t len, uint16_t timeout) {
  ...
  do {
    if (_wire->requestFrom(address, 6 + length + 2)) {
      if (read() & 1) { // check first byte --- status
        break;          // PN532 is ready
      }
    }
...
int8_t PN532_I2C::readAckFrame() {
  ...
  uint16_t time = 0;
  do {
    if (_wire->requestFrom(address, sizeof(PN532_ACK) + 1)) {
      if (read() & 1) { // check first byte --- status
        break;          // PN532 is ready
      }
    }
