...
PN532_SWHSU::PN532_SWHSU() {
  command = 0;
}

void PN532_SWHSU::setSerial(int8_t rxPin, int8_t txPin) {
  SWSerial.begin(115200, SWSERIAL_8N1, rxPin, txPin, false);
  _serial = &SWSerial;
}
...
int8_t PN532_SWHSU::writeCommand(const uint8_t *header, uint8_t hlen,
...
int16_t PN532_SWHSU::readResponse(uint8_t buf[], uint8_t len,
                                  uint16_t timeout) {
  uint8_t tmp[3];
